...
object RelLang {
  ...
  case class OnPatternMatch[K[_], V <: HList](p: Pattern[V], h: V => K[Unit]) extends RelLang[K, Unit]
  ...
  def onPatternMatch[K[_], V <: HList](p: Pattern[V], h: V => K[Unit]): RelLang[K, Unit] = OnPatternMatch(p, h)
  ...
  def onPatternMatchF[F[_[_], _], V <: HList](p: Pattern[V])(h: V => FreeK[F, Unit])(implicit inj: InjectK[RelLang, F]): FreeK[F, Unit] =
    FreeK.lift(onPatternMatch[FreeK[F, ?], V](p, h))
