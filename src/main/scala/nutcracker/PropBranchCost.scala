...
final case class PropBranchCost[C: NonDecreasingMonoid]() extends PropagationBundle with BranchingBundle {
  ...
  def bfsSolver: BFSSolver[Lang, State, Id, λ[A => Val[Promise[A]]], C] =
    new BFSSolver[Lang, State, Id, λ[A => Val[Promise[A]]], C](interpreter, empty[Prg], assess, fetch, getCost)
  ...
  private def fetch: λ[A => Val[Promise[A]]] ~> (State[Prg] => ?) =
    λ[λ[A => Val[Promise[A]]] ~> (State[Prg] => ?)](pa => s => Prop.fetchResult(s._1)(pa).get)
