...
class BFSSolver[C: NonDecreasingMonoid] extends Solver[PropBranchPromCost[C], StreamT[Id, ?]] {
  ...
  @tailrec
  private def unfold[A](heap: Heap[S], pr: Promised[A]): Option[(A, Heap[S])] = heap.uncons match {
    case None => None
    case Some((s, heap1)) => assess(s) match {
      case Failed => unfold(heap1, pr)
      case Done => Some((lang.promStore.get(s).apply(pr).get, heap1))
      case Incomplete(sks) =>
