...
object CellSet {
  ...
  final class InitSyntaxHelper[A] {
    def apply[F[_], Var[_]]()(implicit P: Propagation.Aux0[F, Var]): F[Var[CellSet[Var, A]]] =
  ...
  /** Returns the given set in a CPS style, executing any subsequently
    ...
    */
  def forEach[F[_], Var[_], A](set: Var[CellSet[Var, A]])(implicit P: Propagation.Aux0[F, Var], F: Applicative[F]): IndexedContT[Subscription[F], Unit, F, Var[A]] = {
  ...
  def forEach_[F[_], Var[_], A](set: Var[CellSet[Var, A]])(implicit P: Propagation.Aux0[F, Var], F: Applicative[F]): ContU[F, Var[A]] = {
    val cps = forEach(set)
    ContU(f => cps(f).void)
  }

  def insert[F[_], Var[_], A](ref: Var[A], into: Var[CellSet[Var, A]])(implicit P: Propagation.Aux0[F, Var], dom: Dom[A], F: Functor[F]): F[Unit] =
    ref.observe.by(a =>
  ...
  def insertAll[F[_], Var[_], A](add: Set[Var[A]], into: Var[CellSet[Var, A]])(implicit P: Propagation.Aux0[F, Var], dom: Dom[A], F: Applicative[F]): F[Unit] =
    add.iterator.traverse_(ra => insert(ra, into))

  def include[F[_], Var[_], A](sub: Var[CellSet[Var, A]], sup: Var[CellSet[Var, A]])(implicit P: Propagation.Aux0[F, Var], dom: Dom[A], F: Applicative[F]): F[Unit] =
  ...
  def includeC[F[_], Var[_], A](cps: ContU[F, Var[A]], ref: Var[CellSet[Var, A]])(implicit P: Propagation.Aux0[F, Var], dom: Dom[A], F: Functor[F]): F[Unit] =
    cps(Id(a => insert(a, ref)))

  def collect[F[_], Var[_], A](cps: ContU[F, Var[A]])(implicit P: Propagation.Aux0[F, Var], dom: Dom[A], F: Bind[F]): F[Var[CellSet[Var, A]]] = for {
    res <- init[A]()
    _   <- includeC(cps, res)
  } yield res

  def collectAll[F[_], Var[_], A](cps: ContU[F, Var[A]]*)(implicit P: Propagation.Aux0[F, Var], dom: Dom[A], F: Monad[F]): F[Var[CellSet[Var, A]]] =
    collectAll(cps)

  def collectAll[F[_], Var[_], A](cps: Iterable[ContU[F, Var[A]]])(implicit P: Propagation.Aux0[F, Var], dom: Dom[A], F: Monad[F]): F[Var[CellSet[Var, A]]] =
    collect(ContU.sequence(cps))

  def relBind[F[_], Var[_], Val[_], A, B](sref: Val[CellSet[Var, A]])(f: Var[A] => F[Var[CellSet[Var, B]]])(implicit P: Propagation.Aux1[F, Var, Val], domB: Dom[B], M: Monad[F]): F[Var[CellSet[Var, B]]] = {
