...
final case class Uncons[S[_]](run: `Forall{* -> *}`[λ[K => StateT[Option, S[K], Lst[K]]]]) extends AnyVal { self =>
  def apply[K] = run[K]

  def zoomOut[T[_]](f: `Forall{* -> *}`[λ[K => Lens[T[K], S[K]]]]): Uncons[T] = {
  ...
  def orElse(that: Uncons[S]): Uncons[S] = Uncons[S](new `Forall{* -> *}`[λ[K => StateT[Option, S[K], Lst[K]]]] {
