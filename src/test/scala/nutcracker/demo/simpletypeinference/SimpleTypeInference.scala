...
class SimpleTypeInference[F[_], Propagation <: nutcracker.Propagation[F]](using val P: Propagation) {
  ...
  object BiTypeExpr {
    def apply(f: BiTypeExpr[●, ●], x: TpeCell): F[TpeCell] =
      ???
    ...
    f match {
      ...
      case fix: FixF[g] =>
        ...
        for {
          ...
          gfg <- BiTypeExpr.apply(g, fg)
          _ <- unifyTpes(a, gfg)
          _ <- unifyTpes(b, fg)
        } yield ()

      case unfix: UnfixF[g] =>
        val TypeFun(r, g) = TypeTag.toTypeFun(fix.f)
        for {
          g <- BiTypeExpr.unroll(TypeTag.toTypeFun(unfix.f))
          fg <- TpeCell.fix(r, g.annotation)
          gfg <- BiTypeExpr.apply(g, fg)
          _  <- unifyTpes(a, fg)
          _  <- unifyTpes(b, gfg)
        } yield ()
