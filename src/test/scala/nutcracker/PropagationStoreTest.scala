...
class PropagationStoreTest extends FunSuite {
  ...
  test("diff accumulation") {
    val prg = for {
      ...
      log <- newLog[CMDelta[DecSet[Int]]]
      _   <- domTriggerF(ref)(s => (None, Some((s: DecSet[Int], d: CMDelta[DecSet[Int]]) => fireReload(log.write(d)))))
    ...
    assertResult(List(\/-(Diff(DecSet(1, 2, 4, 5)))))(log)
  }
  ...
  test("delta is published when registering new trigger") {
    ...
    val (store, logRef) = PropagationStore.interpreter.freeInstance.apply(prg)(PropagationStore.empty[FreeK[PropagationLang, ?]])
    val log = store.fetch(logRef)
    assertResult(List(\/-(Diff(DecSet(2, 3)))))(log)
  }
